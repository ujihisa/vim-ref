...
-   **"sub":** the `-` operation. Behavior similar to the "add"
    operation.
...
Unless otherwise noted, any function that accepts valid indices can also
...
errors, [`lua_getstack`](#lua_getstack) returns 1; when called with a
level greater than the stack depth, it returns 0.
...
[`lua_getupvalue`](#lua_getupvalue) gets the index `n` of an upvalue,
pushes the upvalue's value onto the stack, and returns its name.
`funcindex` points to the closure in the stack. (Upvalues have no
...
any other field in `ar`, the hook must call
[`lua_getinfo`](#lua_getinfo). For return events, `event` can be
...
in this case, it is useless to call [`lua_getinfo`](#lua_getinfo).
...
`LUA_MASKCALL`, `LUA_MASKRET`, `LUA_MASKLINE`, and `LUA_MASKCOUNT`. The
`count` argument is only meaningful when the mask includes
`LUA_MASKCOUNT`. For each event, the hook is called as explained below:
...
Parameters `ar` and `n` are as in [`lua_getlocal`](#lua_getlocal) (see
[`lua_getlocal`](#lua_getlocal)). [`lua_setlocal`](#lua_setlocal)
...
from the stack. Parameters `funcindex` and `n` are as in the
[`lua_getupvalue`](#lua_getupvalue) (see
...
the error message is formatted for arguments (e.g.,
...
Adds the character `c` to the buffer `B` (see
...
[`luaL_Buffer`](#luaL_Buffer)). The string may contain embedded zeros.
...
[`luaL_Buffer`](#luaL_Buffer)). The string may not contain embedded
zeros.
...
Checks whether the function argument `narg` is a number and returns this
number cast to a [`lua_Integer`](#lua_Integer).
...
registry (see [`luaL_newmetatable`](#luaL_newmetatable)).
...
it has an extra error code `LUA_ERRFILE` if it cannot open/read the
file.
...
panic function (see [`lua_atpanic`](#lua_atpanic)) that prints an error
message to the standard error output in case of fatal errors.
...
an `int`. If this argument is absent or is **nil**, returns `d`.
...
If the function argument `narg` is a number, returns this number cast to
a [`lua_Integer`](#lua_Integer). If this argument is absent or is
...
If the function argument `narg` is a number, returns this number cast to
a `long`. If this argument is absent or is **nil**, returns `d`.
Otherwise, raises an error.
...
copy a string to be added to buffer `B` (see
[`luaL_Buffer`](#luaL_Buffer)). After copying the string into this space
you must call [`luaL_addsize`](#luaL_addsize) with the size of the
...
[`luaL_ref`](#luaL_ref) returns the constant `LUA_REFNIL`. The constant
`LUA_NOREF` is guaranteed to be different from any reference returned by
[`luaL_ref`](#luaL_ref).
...
[`luaL_register`](#luaL_register). `name` is the function name and
`func` is a pointer to the function. Any array of
[`luaL_Reg`](#luaL_Reg) must end with an sentinel entry in which both
...
functions in the list `l` (see [`luaL_Reg`](#luaL_Reg)) into the table
...
where `location` is produced by [`luaL_where`](#luaL_where), `func` is
the name of the current function, and `rt` is the type name of the
actual argument.
...
-   **"setpause":** sets `arg` as the new value for the *pause* of the
    collector (see [§2.10](#2.10)). Returns the previous value for
    *pause*.
-   **"setstepmul":** sets `arg` as the new value for the *step
    multiplier* of the collector (see [§2.10](#2.10)). Returns the
...
[`setfenv`](#pdf-setfenv) to change environments.)
...
Similar to [`load`](#pdf-load), but gets the chunk from the given
string.
...
numerical **for** or the [`ipairs`](#pdf-ipairs) function.)
...
Returns three values: the [`next`](#pdf-next) function, the table `t`,
...
using the [`tostring`](#pdf-tostring) function to convert them to
...
output, use [`string.format`](#pdf-string.format).
...
exponent part (see [§2.1](#2.1)). In other bases, only unsigned integers
are accepted.
...
defined by the length operator (see [§2.5.5](#2.5.5)).
...
This function is similar to [`pcall`](#pdf-pcall), except that you can
...
library and come inside the table `coroutine`. See [§2.11](#2.11) for a
...
environment: [`require`](#pdf-require) and [`module`](#pdf-module).
...
[`package.loaders`](#pdf-package.loaders) array. By changing this array,
we can change how `require` looks for a module. The following
...
*all-in-one* loader (see [`package.loaders`](#pdf-package.loaders)).
...
stored at [`package.path`](#pdf-package.path). A path is a sequence of
...
given by the variable [`package.cpath`](#pdf-package.cpath). For
...
function, so that a later [`loadstring`](#pdf-loadstring) on this string
returns a copy of the function. `function` must be a Lua function
without upvalues.
...
`n+1`, where `n` is the length of the table (see [§2.5.5](#2.5.5)), so
...
[`io.lines`](#pdf-io.lines), this function does not close the file when
the loop ends.)
...
-   **"full":** full buffering; output operation is performed only when
    the buffer is full (or when you explicitly `flush` the file (see
...
[`tostring`](#pdf-tostring) or [`string.format`](#pdf-string.format)
before `write`.
...
active local variable.) The function returns **nil** if there is no
local variable with the given index, and raises an error when called
with a `level` out of range. (You can call
...
simply `lua`, is provided with the standard distribution. The
...
the given *args* as string arguments. When called without arguments,
`lua` behaves as `lua -v -i` when the standard input (`stdin`) is a
terminal, and as `lua -` otherwise.

Before running any argument, the interpreter checks for an environment
variable `LUA_INIT`. If its format is `@filename`, then `lua` executes
...
and finally run the file `script.lua` with no arguments. (Here `$` is
the shell prompt. Your prompt may be different.)
...
interpreter name plus the options) go to negative indices. For instance,
in the call
...
`luaconf.h`). However, all these compatibility options will be removed
...
-   Function `string.gfind` was renamed
    [`string.gmatch`](#pdf-string.gmatch). (See compile-time option
    `LUA_COMPAT_GFIND` in `luaconf.h`.)
-   When [`string.gsub`](#pdf-string.gsub) is called with a function as
    its third argument, whenever this function returns **nil** or
    **false** the replacement string is the whole match, instead of the
    empty string.
-   Function `table.setn` was deprecated. Function `table.getn`
    corresponds to the new length operator (`#`); use the operator
    instead of the function. (See compile-time option `LUA_COMPAT_GETN`
    in `luaconf.h`.)
-   Function `loadlib` was renamed
    [`package.loadlib`](#pdf-package.loadlib). (See compile-time option
    `LUA_COMPAT_LOADLIB` in `luaconf.h`.)
-   Function `math.mod` was renamed [`math.fmod`](#pdf-math.fmod). (See
    compile-time option `LUA_COMPAT_MOD` in `luaconf.h`.)
-   Functions `table.foreach` and `table.foreachi` are deprecated. You
    can use a for loop with `pairs` or `ipairs` instead.
-   There were substantial changes in function [`require`](#pdf-require)
    due to the new module system. However, the new behavior is mostly
    compatible with the old, but `require` gets the path from
    [`package.path`](#pdf-package.path) instead of from `LUA_PATH`.
-   Function [`collectgarbage`](#pdf-collectgarbage) has different
    arguments. Function `gcinfo` is deprecated; use
...
-   Function `lua_open` was replaced by [`lua_newstate`](#lua_newstate)
    to allow the user to set a memory-allocation function. You can use
    [`luaL_newstate`](#luaL_newstate) from the standard library to
    create a state with a standard allocation function (based on
    `realloc`).
-   Functions `luaL_getn` and `luaL_setn` (from the auxiliary library)
    are deprecated. Use [`lua_objlen`](#lua_objlen) instead of
    `luaL_getn` and nothing instead of `luaL_setn`.
-   Function `luaL_openlib` was replaced by
